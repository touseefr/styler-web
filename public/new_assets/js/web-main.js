...
(function () {
    ...
    function WatchListController($state, AccountModel, toaster, $location, Spinner) {
        ...
        self.watchlistdelete = function ($event, watchlist_id, watchtype) {
            ...
            Spinner.start();
            AccountModel.watchlistdelete({id: watchlist_id, watchtype: watchtype}).then(function (responseData) {
                Spinner.stop();
...
(function () {
    ...
    /* @ngInject */
    function UserController($state, SuburbsModel, AccountModel, ResolveData, toaster, CategoriesModel, Spinner) {
        var self = this;
        init();
        self.temp1 = '';
        self.temp2 = '';
        ...
        function init() {
            ...
            self.userModel = {
                ...
                'jobtype': (ResolveData.user_info && ResolveData.user_info.jobtype) ? ResolveData.user_info.jobtype : null,
                'jobposition': (ResolveData.user_info && ResolveData.user_info.jobposition) ? ResolveData.user_info.jobposition : null,
            ...
            AccountModel.getaccountcats().then(function (response) {
                ...
                if (self.userModel.cat_id != null) {
                    ...
                    for (var i = 0; i < self.temp2.length; i++) {
                        self.temp.push({id: self.temp1[i], name: self.temp2[i]});
                    }
                }
    ...
    UserController.$inject = ["$state", "SuburbsModel", "AccountModel", "ResolveData", "toaster", "CategoriesModel", "Spinner"];
    //end of controller
...
(function () {
    ...
    /* @ngInject */
    function ListListingController(CategoriesModel, ListingModel, Listing_type, $location, moment, toaster, Spinner, $state, $http) {
        var self = this;
        self.listing = '';
        self.adddeals = '';
        init();
    ...
    ListListingController.$inject = ["CategoriesModel", "ListingModel", "Listing_type", "$location", "moment", "toaster", "Spinner", "$state", "$http"];
    //end of controller
    angular
            .module('BeautyCollective.Listing')
    ...
    function BookingModuleController($http, Spinner, $state, toaster, $location) {
        ...
        function getBooking() {
            $http({
                ...
                if (response.data.bookingData.length > 0) {
                    _self.booking_chuncks = response.data.bookingData;
                    _self.bookings = _self.booking_chuncks[_self.current_page];
                    _self.total_records = response.data.total_records;
                    _self.total_pages = numberOfPages();
                }
            }
            );
        }
        ...
        _self.cancelBooking = function (booking_id) {

            if (_self.disabled_cancel_button == 1) {
                Spinner.start();
                $http({
                    method: 'POST',
                    url: '/cancelbooking',
...
(function () {
    ...
    /* @ngInject */
    function SubscriptionController($scope, $state, $window, Laravel, ResolveData, checkusersubscription, $http, $location, $timeout) {
        ...
        _self.changepackage = function (plan_id) {
           ...
           _self.changemodule = 1;
            var stripe = Stripe('pk_test_XvmKBqleTGuZGhrvSSYVIwjx');
...
// Handle form submission.
            ...
            if (plan_id != _self.userplanid) {
                $http.post("changepackage", {new_plan_id: plan_id})
    ...
    SubscriptionController.$inject = ["$scope", "$state", "$window", "Laravel", "ResolveData", "checkusersubscription", "$http", "$location", "$timeout"];
    angular
            .module('BeautyCollective.Account')
...
(function () {
    ...
    /* @ngInject */
    function AccountSettingsController($scope, $sce, $state, AccountModel, ResolveData, toaster, SuburbsModel, $http, Spinner, CategoriesModel, $location, CSRF_TOKEN, NotificationModel) {
        var self = this, team_id, imgid, team_member_id;
        init();
        ...
        function init() {
             ...
             * @return {void}
             */
            team_member_id = $state.params.team_member_id || null;
            ...
            if ($state.current.name == 'settings.paymethod') {
        ...
        function SetTeamMemberInform() {
            AccountModel.getteammemberbyid({team_member_id: team_member_id}).then(function (response) {
        ...
        self.DeleteExistingTMProfilePic = function ($event, image_id, member_id) {
            $event.preventDefault();
            AccountModel.TeamMPic({image_id: image_id, member_id: member_id}).then(function (responseData) {
        ...
        self.updateTMember = function () {            
            var updateimage = angular.copy([self.teams]), resource;            
            var upmember11 = angular.copy(self.updatetm.data), resource;            
        ...
        self.DeleteUploadTMpic = function ($event, image_id) {
            $event.preventDefault();            
            AccountModel.UploadTeamMPic({image_id: image_id}).then(function (responseData) {
                toaster.pop('success', "Image Deleted", "Image has been deleted successfully.");
                $state.go($state.current, {}, {reload: true});                
            }, function (errorResponse) {
                console.log('unable to delete list : ', errorResponse);
        ...
        self.DeleteTeamMember = function ($event, team_id) {
            $event.preventDefault();
            Spinner.start();
            AccountModel.DeleteTeam({id: team_id}).then(function (responseData) {
                Spinner.stop();
        ...
        self.CreatMyCoverletterAndResume = function () {
            Spinner.start();
            var coverletter = angular.copy(self.mycoverletter), resource;                        
            var resumefileId = angular.copy(self.usergallery), resource;
        ...
        self.SavePaymentMethod = function () {
            ...
            var PaymentMethod = angular.copy(self.payment), resource;
        ...
        self.DeleteMygalleryImage = function ($event, list_id, cat_id) {
            $event.preventDefault();
            MyGalleryImages();            
            AccountModel.DeleteMygalleryImage({id: list_id, cat_id: cat_id}).then(function (responseData) {
                toaster.pop('success', "Image Deleted", "Image has been deleted successfully.");
                $state.go($state.current, {}, {reload: true});                
            }, function (errorResponse) {
                console.log('unable to delete list : ', errorResponse);
        ...
        self.DeleteMyResume = function ($event, list_id) {
            $event.preventDefault();            
            GetMyCLandResume();
            AccountModel.DeleteMyResumeFile({id: list_id}).then(function (responseData) {
        ...
        self.DeleteMyvideogallery = function ($event, list_id, cat_id) {
            $event.preventDefault();
            MyVideoGallery();            
            AccountModel.DeleteMyvideogallery({id: list_id, cat_id: cat_id}).then(function (responseData) {
        ...
        self.CreatMyVideoGallery = function () {
            ...
            Spinner.start();
            var args = angular.copy(self.gallery), resource;            
        ...
        self.CreatMyGallery = function () {            
            var ctgallery = angular.copy(self.gallery), resource;
            var img = angular.copy(self.usergallery), resource;
        ...
        self.createteam = function () {
            var teams = angular.copy(self.teams), resource;
       ...
       /********************** upload team image ******************/
        ...
        self.teamcancelFile = function ($event, list_id) {
            $event.preventDefault();
            AccountModel.teamcancelFile({id: list_id}).then(function (responseData) {
    ...
    AccountSettingsController.$inject = ["$scope", "$sce", "$state", "AccountModel", "ResolveData", "toaster", "SuburbsModel", "$http", "Spinner", "CategoriesModel", "$location", "CSRF_TOKEN", "NotificationModel"];
    //end of controller
...
(function () {
    ...
    function CreateListingController(CategoriesModel, $state, $q, toaster, ListingModel, utilFactory, Listing_type, SuburbsModel, Spinner, moment) {
        ...
        self.savelisting = function () {
            ...
            resource.then(function (successResponse) {
                ...
                $state.go('listing.list', {}, {reload: true});
            }, function (errorResponse) {
                Spinner.stop();
                $state.go('listing.list', {}, {reload: true});
    ...
    CreateListingController.$inject = ["CategoriesModel", "$state", "$q", "toaster", "ListingModel", "utilFactory", "Listing_type", "SuburbsModel", "Spinner", "moment"];
    //end of controller
...
(function () {
    ...
    /* @ngInject */
    function AccountBusinessInfoController($http, $state, AccountModel, Laravel, AccountFactory, ResolveData, toaster, SuburbsModel, CategoriesModel, Spinner) {
        var self = this, faq_id, service_id, brand_id, roles = [];
        ...
        function init() {
             ...
             */
             ...
             self.userBusinessModel = ResolveData.user_business ? {
                ...
                'iagree': (ResolveData.user_business.iagree == 1) ? true : false
            } : {
                'id': ResolveData.id
            };
            ...
            if (ResolveData.user_business.abn && ResolveData.user_business.abn_name) {
        ...
        function getbusinessfaq() {
            ...
            AccountModel.getfaqbyid({faq_id: faq_id}).then(function (response) {
        ...
        function SetBusinessBrandInForm() {
            ...
            AccountModel.getbussbrandbyid({brand_id: brand_id}).then(function (response) {                
        ...
        function SetBusinessServiceInForm() {
            ...
            AccountModel.getbussservicebyid({service_id: service_id}).then(function (response) {
        ...
        self.faqdelete = function ($event, list_id) {
            ...
            Spinner.start();
            AccountModel.faqdelete({id: list_id}).then(function (responseData) {
        ...
        self.busbranddelete = function ($event, list_id) {
            $event.preventDefault();
            Spinner.start();
            AccountModel.bussbranddelete({id: list_id}).then(function (responseData) {
        ...
        self.busservicedelete = function ($event, list_id) {
            ...
            Spinner.start();
            AccountModel.busservicedelete({id: list_id}).then(function (responseData) {
        ...
        self.businesfaq = function () {
            Spinner.start();
            var _business = angular.copy(self.faq), resource;
        ...
        self.updatebusinesfaq = function () {
            ...
            var _business2 = angular.copy(self.updateBussFaq.data), resource;            
        ...
        self.UpdateBusinesBrand = function () {
            Spinner.start();
            var _business2 = angular.copy(self.BussBrandSetVal.data), resource;
            console.log(_business2);
        ...
        self.UpdateBusinesService = function () {
            Spinner.start();
            var _business2 = angular.copy(self.BussServiceSetVal.data), resource;
            console.log(_business2);
        ...
        self.create_businessbrand = function () {
            ...
            Spinner.start();
            var businessbrand = angular.copy(self.brand), resource;            
        ...
        self.create_businesservice = function () {
            ...
            Spinner.start();
            var businessservices = angular.copy(self.services), resource;            
        ...
        self.updateUserSocialAccounts = function (obj) {

            var data = $("#social_media_form").serialize();
        ...
        self.removeLogo = function ($event, user_id) {
            ...
            if (confirm('Are you sure you want to delete your logo?')) {
                Spinner.start();
                AccountModel.deleteLogo({id: user_id}).then(function (responseData) {
                    self.userBusinessModel.logo = '';
                    ResolveData.profilepic.path = '';
                    ResolveData.profilepic.name = '';
        ...
        self.removeVideo = function ($event, user_id) {
            ...
            if (confirm('Are you sure you want to delete your video?')) {
                Spinner.start();
                AccountModel.deleteVideo({id: user_id}).then(function (responseData) {
        ...
        self.validateAbn = function () {
            ...
            $http({
                method: 'get',
                url: "/abnValidation",
                params: {abn: self.userBusinessModel.abn, cname: self.userBusinessModel.abn_name}
            }).then(function mySuccess(response) {
                var info = response.data;
                if (info.status === "200") {
                    self.validabncheck = 1;
                    self.showerror = 0;
                    toaster.pop('success', "ABN Detail Save", "ABN Details has been saved Successfully.");
                } else {
                    self.showerror = 1;
        ...
        self.checkkeyup = function () {
            self.validabncheck = 0;
            if (ResolveData.user_business.abn == self.userBusinessModel.abn && ResolveData.user_business.abn_name == self.userBusinessModel.abn_name) {
                self.validabncheck = 1;
            }
        };
    ...
    AccountBusinessInfoController.$inject = ["$http", "$state", "AccountModel", "Laravel", "AccountFactory", "ResolveData", "toaster", "SuburbsModel", "CategoriesModel", "Spinner"];
    //end of controller
...
(function () {
    ...
    function DashboardController($scope, $state, $window, Laravel, ResolveData, checkusersubscription) {
        ...
        function init() {
            ...
            self.planid = (ResolveData.user.user_subscription.length > 0) ? ResolveData.user.user_subscription[0].plan_id : '1';
        }
...
//            var factory_date=objdate.toISOString();
//            objdate.setHours(18);
//            objdate.setMinutes(0);
//            var factory_date_close=objdate.toISOString();
            return {
                monday: {
                    open: moment().utcOffset(5).set({hour: 9, minute: 0, second: 0, millisecond: 0}).toISOString(),
                    close: moment().utcOffset(5).set({hour: 18, minute: 0, second: 0, millisecond: 0}).toISOString()
                },
                tuesday: {
                    open: moment().utcOffset(5).set({hour: 9, minute: 0, second: 0, millisecond: 0}).toISOString(),
                    close: moment().utcOffset(5).set({hour: 18, minute: 0, second: 0, millisecond: 0}).toISOString()
...
(function () {
    'use strict';
    angular.module('BeautyCollective.Account')
            .factory('checkusersubscription', function () {
                return {
                    checkstatus: function (roletype, planid, pl = 0) {
                        var ret = false;
                        if (pl == 0) {
                            if (roletype == "ServiceProvider" || roletype == "Distributor") {
...
(function () {
    ...
    /* @ngInject */
    function AccountController($state, ResolveData, AccountModel, toaster, Spinner, $scope, checkusersubscription) {
        var self = this;
        init();
        ...
        function init() {
            ...
            self.pageSize = 4;
            self.userRole = ResolveData.user.roles[0].name;
            $scope.plainid = self.planid = (ResolveData.user.user_subscription.length > 0) ? ResolveData.user.user_subscription[0].plan_id : '1';
    ...
    AccountController.$inject = ["$state", "ResolveData", "AccountModel", "toaster", "Spinner", "$scope", "checkusersubscription"];
    //end of controller
...
(function () {
    ...
    function BusinessController($location, moment, toaster, Spinner, $state) {
        ...
        function init() {
            console.log('BusinessController initialized');
        }
...
(function () {
    ...
    myApp.controller('HomeMainController', function ($scope) {
        ...
        $scope.likeGallery = function (user_id, asset_id, ele_id) {
            var resource;
            var data = {"user_id": user_id, "asset_id": asset_id};
            resource = $.ajax({
                method: "POST",
                url: "/addgallerylike",
                data: data
        ...
        $scope.paginateGallery = function () {
            console.log('refresh gallery called');
...
(function () {
    ...
    function BusinessNewController($scope, Spinner, toaster) {
        ...
        $scope.ReportBusiness = function (type, item_title, item_id) {
            ...
            var success = false;
            var data = {"type": type, "item_title": item_title, "item_id": item_id};
            Spinner.start();
...
(function () {
    ...
    function ShortlistjobController($sanitize, $q, Logger, Spinner, ShortlistjobModel, toaster) {
        ...
        self.shortlistJob = function () {
            ShortlistjobModel.ShortlistJob({}, {id: self.id}).then(function (responce) {
                if (responce.status) {
    ...
    angular
            ...
            .directive('widgetRatingHome', [function () {
                    return {
                        ...
                        templateUrl: '/apps/widgets/rating/rating_home.html',
                        controller: 'RatingController',
                        controllerAs: 'self',
                        bindToController: true, //required in 1.3+ with controllerAs
                        link: function (scope, element, iAttrs, ngModel) {
...
(function () {
    ...
    function SearchResultController($scope, $sanitize, $q, Logger, Spinner, SearchResultModel, $location, moment, CategoriesModel, SuburbsModel, userAuthentication, $window) {
        ...
        self.selectTab = function (tab, index) {
            ...
            if (tab.id == "job" || tab.id == "distributor" || tab.id == "serviceprovider" || tab.id == "deal" || tab.id == "businessforsale" || tab.id == "schoolcolleges") {
            ...
            if (tab.id == "gallery") {
                ...
                $window.open('/allclassified', '_blank');
            }
        ...
        self.selectTabOther = function (tab, index) {
            ...
            if (tab.id == "deal" || tab.id == "serviceprovider") {
                _.each(self.tabs, function (list, index) {
    ...
    SearchResultController.$inject = ["$scope", "$sanitize", "$q", "Logger", "Spinner", "SearchResultModel", "$location", "moment", "CategoriesModel", "SuburbsModel", "userAuthentication", "$window"];
})();
...
(function () {
    ...
    function ListingByCatController($scope, $q, Logger, Spinner, $location, moment, CategoriesModel, SuburbsModel) {
        ...
        function getRecords() {
            ...
            var data = {'type': type, 'page': self.qs.page, 'is_api': true};
...
(function () {
    ...
    function GalleryController($scope, $q, Logger, Spinner, $location, moment, CategoriesModel, SuburbsModel) {
        ...
        function getRecords() {
            ...
            var data = {'page': self.qs.page, 'is_api': true};
...
(function () {
    ...
    function SearchController($scope, $sanitize, $q, Logger, Spinner, SearchModel, SuburbsModel, $location, $http) {
        ...
        self.searchTimerComplete = function (str) {
            if (str.length >= self.minLength) {
                if (self.localData) {
                    ...
                    SearchModel.findAll(data).then(function (responce) {
                        self.selected = false;
                        self.searching = false;
                        self.results = responce.list;                       
                    }, function (error) {
                        Spinner.stop();
                    });
    ...
    SearchController.$inject = ["$scope", "$sanitize", "$q", "Logger", "Spinner", "SearchModel", "SuburbsModel", "$location", "$http"];
})();
...
(function () {
    ...
    function ActivatejobController($sanitize, $q, Logger, Spinner, ActivatejobModel, toaster) {
        ...
        self.activateJob = function () {
            ActivatejobModel.ActivateJob({}, {id: self.id, action: self.showlink}).then(function (responce) {
                if (responce.status) {
...
(function () {
    ...
    function ApplyjobController($sanitize, $q, Logger, Spinner, ApplyjobModel, toaster) {
        ...
        self.applyJob = function () {
            ...
            if ((self.coverLetter.asset_id !== '-') && (self.coverLetter.asset_id == '-')) {
...
(function () {
    ...
    function AuthController($sanitize, $http, Logger, Spinner, SuburbsModel, $scope, stripeInfo, $location, AccountFactory) {
        ...
        self.userSchema = {
            ...
            suburb: '',
            locations: {location: 'Address? (suburb,', state: " postcode", postcode: ", or city)"},
            planid: ''
        };
        /**
         * [login description]
