...
class UserController extends Controller {
    ...
    public function index() {
        return view('backend.access.index')
                        ->withUsers($this->users->getUsersPaginated(/* config('access.users.default_per_page') */ 1000, 1));
    }
    ...
    public function store(StoreUserRequest $request) {
        ...
        return redirect('admin/access/users')->withFlashSuccess(trans("alerts.users.created"));
    }
    ...
    public function update($id, UpdateUserRequest $request) {
        $this->users->update($id, $request->except('assignees_roles', 'permission_user', 'pre_package_id', 'packeage'), $request->only('assignees_roles'), $request->only('permission_user'), $request->only('pre_package_id', 'packeage')
        );
    ...
    public function getSearchUser(Request $request) {
        $input = $request->all();
        $user_Data = $this->users->getUserSearch($input);
        $array_return = array();
        if ($user_Data) {
            $array_return = array("status" => "200", "data" => $user_Data, "msg" => "Successfully Fetch Records");
        } else {
            $array_return = array("status" => "210", "data" => array(), "msg" => "Record Not Found");
        }
        return $array_return;
    }
